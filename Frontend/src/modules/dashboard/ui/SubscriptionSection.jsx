...
function SubscriptionSection() {
  ...
  const currentSubscription = useMemo(() => {
    const isPremium =
      (subscription?.planType || "").toUpperCase() === "PREMIUM" &&
    ...
    return {
      ...
      features: isPremium
        ...
        : ["Up to 3 job posts", "Basic applicant search", "Standard support"],
    };
  }, [subscription]);
  ...
  const handleUpgradePlan = (plan) => {
    const run = async () => {
      ...
      if (!companyEmail) {
        showError(
          "Missing company email. Please update your profile email first."
        );
        return;
      }
        ...
        <div
          className={`${
            currentSubscription.planType === "Premium"
              ? "bg-primary"
              : "bg-gray-800"
          } text-white p-6 border-b-4 border-black`}
        >
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className="flex items-center space-x-3 mb-2">
                ...
                <span
                  className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold uppercase ${getStatusColor(
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b-2 border-black">
                ...
                <th className="px-4 py-3 text-right font-black uppercase text-sm">
                  Amount
                </th>
                <th className="px-4 py-3 text-center font-black uppercase text-sm">
                  Status
                </th>
                <th className="px-4 py-3 text-center font-black uppercase text-sm">
                  Invoice
              ...
              <div className="mb-6">
                ...
                <ul className="space-y-2 mb-4">
                  {currentSubscription.features.map((feature, index) => (
                    <li
                      key={index}
                      className="flex items-start font-semibold text-gray-600"
                    >
                      <svg
                ...
                <div className="bg-yellow-50 border-2 border-yellow-500 p-4">
                  <p className="font-bold text-sm text-yellow-800">
                    ⚠️ Your subscription will remain active until{" "}
