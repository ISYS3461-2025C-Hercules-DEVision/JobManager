  ...
  const searchApplicants = async (page = 1) => {
    try {
      ...
      if (!filters.degree || filters.degree.trim() === "") {
        const allDegrees = DEGREE_TYPES.filter((d) => d.value !== "") // Exclude the "All Degrees" option
          .map((d) => d.value);
    ...
    <div>
          ...
          <div className="flex items-end">
            <label className="flex items-center space-x-2 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  setFilters({ ...filters, matchAllSkills: e.target.checked })
                }
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-black uppercase">
          {loading
            ? "Searching..."
            : `Found ${pagination.totalElements} Applicants`}
        </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {applicants.map((applicant) => (
              <div
                ...
                className="bg-white border-4 border-black hover:shadow-lg transition-shadow"
              >
                <div className="p-6">
                  {/* Avatar and Name */}
